 ...
 */
export async function createSilenceListenerNode(
	context,
	nChannels,
	options = {}
) {
	...
	let silenceThreshold =
		options.silenceThreshold || Math.floor(44100 / batchSize);
	...
	if (window.AudioWorklet !== undefined)
		backend = await createWorkletBackend(
			context,
			nInputs,
			nOutputs,
			nChannels,
		...
		backend = new ScriptProcessorBackend(
			context,
			batchSize,
			nInputs,
			nOutputs,
...
function validate(context, nInputs, nOutputs, nChannels, batchSize) {
	...
	if (nInputs != nOutputs)
		throw "SilenceListenerNode only support nInputs === nOutputs";
	if (nInputs < 1) throw "nInputs must be >= 1";
	if (nChannels < 1) throw "nChannels must be >= 1";
	if (![256, 512, 1024, 2048, 4096, 8192, 16384].includes(batchSize))
		throw "batch size must be one of [256, 512, 1024, 2048, 4096, 8192, 16384]";
}
